...
def launch_ec2_workers(access_key, secret_key, token, bucket, dataset_key, worker_script_key, algo, n_clusters, num_workers, params_json):
    ec2 = boto3.resource('ec2', region_name=REGION)
    instances = []
    
    print(f"Launching {num_workers} EC2 worker instances in parallel...")
    for i in range(num_workers):
        token_line = f"aws_session_token = {token}" if token else ""
...
# 1. Configure AWS credentials for root (so aws s3 cp can run) and ec2-user
mkdir -p /root/.aws
...
# 2. Download code from S3
aws s3 cp s3://{bucket}/{worker_script_key} /home/ec2-user/worker.py
chown ec2-user:ec2-user /home/ec2-user/worker.py

# 3. Execute clustering script passing tuned parameters
echo "=== Running worker script for batch {i} ==="
su - ec2-user -c "python3 /home/ec2-user/worker.py --bucket {bucket} --s3-key {dataset_key} --batch-index {i} --batch-count {num_workers} --algo {algo} --n-clusters {n_clusters} --params '{params_json}'"
...
def wait_for_s3_results(s3_client, bucket, num_workers):
...
def main():
    ...
    if args.mode == "local":
        ...
        dataset_s3_key = f"dataset/{filename}.csv"
        worker_script_s3_key = "code/worker.py"
        output_local_path = f"ClusteringOutput/{filename}_{args.algo}_distributed.csv"
        
        instances = []
        try:
            upload_file_to_s3(s3_client, args.file_path, args.bucket, dataset_s3_key)
            upload_file_to_s3(s3_client, "DistributedACE/worker.py", args.bucket, worker_script_s3_key)
            
            access_key, secret_key, token = get_local_aws_credentials()
            
            instances = launch_ec2_workers(
                access_key, secret_key, token, args.bucket, dataset_s3_key, 
                worker_script_s3_key, args.algo, args.n_clusters, args.workers, params_json
            )
            
            wait_for_s3_results(s3_client, args.bucket, args.workers)
